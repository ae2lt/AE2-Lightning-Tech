...
void main() {
    ...
    float energy = 0.58 + fresnel * 0.18 + pulse * 0.04 + dataLines * 0.10;
    vec3 silver = vec3(0.82, 0.90, 0.92);
    vec3 surfaceColor = mix(vertexColor.rgb, silver, fresnel * 0.24 + dataLines * 0.10);
    fragColor = vec4(surfaceColor * energy * 0.72, vertexColor.a);
}
