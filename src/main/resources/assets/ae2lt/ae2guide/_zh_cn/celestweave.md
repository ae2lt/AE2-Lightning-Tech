...
两种飞行模块共用原版式操作：空中单击空格展开翼装滑翔，快速双击空格切换悬停。滑翔时持续按住空格会沿视线方向推进；悬停时空格上升、Shift 下降。同时按住 Shift 与空格时垂直输入互相抵消，并对其他模组表现为潜行状态。只有相位飞行可配置相位模式：**关闭**只关闭穿墙而不关闭飞行，**仅创造飞行**只允许悬停穿墙并让滑翔保持正常碰撞，**创造加鞘翅**允许悬停与滑翔穿墙。飞行锁定由胸甲的相位锁定模块统一控制。
