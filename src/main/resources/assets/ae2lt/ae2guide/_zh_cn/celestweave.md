...
| 防激光模块 | 完全吸收 Mekanism 激光；按当前 Mekanism 能量换算比例，将实际吸收的焦耳转化为 FE，依次充入胸甲与整套苍穹织雷 |
| 相位锁定模块 | 提供三个可独立开关的子功能：盔甲锁定、阻止外力拉动、阻止外部传送，详见下文 |
...
相位锁定模块以胸甲为控制器，在设备中枢中提供三个可独立开关的子功能：

* **盔甲锁定：** 将当前穿戴的整套苍穹织雷转移到玩家专属的四个私有相位槽；各护甲栏仅保留无数据的相位锁定占位符。真护甲在私有槽中照常运行，锁定期间新穿上的苍穹织雷部件也会自动纳入。占位符没有附魔光效，无法取出或交易；单个占位符被替换或移除时，消耗 1,000,000 FE 与 16 极高压闪电原位重建，支付失败则整体解除锁定并穿回全部真护甲。
`你可以理解为灵魂绑定`
...
多数运行中的普通模块每 tick 额外消耗 1 高压闪电。创造飞行在飞行时消耗更多高压闪电；相位飞行每 tick 消耗极高压闪电。护盾、净化与不死在短时间内连续触发时，消耗会逐次提高。胸甲安装过载内核模块后，整套护甲的 极高压闪电 缺口可以按 16 高压闪电 = 1 极高压闪电 代偿。
