...
| 相位飞行模块 | 提供悬停与相位翼滑翔；相位模式统一决定飞行时是否穿墙 |

创造飞行与相位飞行属于同一安装组，护腿上只能安装其中一种。相位飞行安装后默认停用，需要在设备中枢中启用。

相位飞行沿用原版飞行操作：空中单击空格展开相位翼滑翔，快速双击空格切换悬停。滑翔时持续按住空格会沿视线方向推进；悬停时空格上升、Shift 下降。同时按住 Shift 与空格时垂直输入互相抵消，并对其他模组表现为潜行状态。相位模式开启时，悬停与相位翼滑翔均可穿过方块；关闭后两种飞行仍可使用，但恢复正常碰撞。飞行锁定开启时，非相位悬停落地不会自动退出，外部系统也不能更改悬停状态；关闭后恢复原版着陆与外部控制行为。
