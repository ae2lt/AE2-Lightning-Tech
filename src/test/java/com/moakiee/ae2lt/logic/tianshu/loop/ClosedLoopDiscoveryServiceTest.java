...
class ClosedLoopDiscoveryServiceTest {
    ...
    void recognizesSinglePatternReturnedCatalystLoop() {
        ...
        assertEquals(1, amount(candidates.get(0).analysis().seeds(), catalyst));
        assertEquals(4, amount(candidates.get(0).analysis().externalInputs(), material));
    }
    ...
    void findsValidCoefficientBeyondTheLegacyBound() {
        ...
        assertEquals(List.of(1L, 9L), java.util.Arrays.stream(
                candidates.get(0).coefficients()).boxed().toList());
        assertEquals(9, amount(candidates.get(0).analysis().seeds(), intermediate));
        assertEquals(1, amount(candidates.get(0).analysis().netOutputs(), target));
    }
    ...
    void discoversLoopThroughANonFirstSubstitutionCandidate() {
        ...
        assertEquals(1, candidates.size());
        assertEquals(2, candidates.get(0).members().size());
        assertEquals(1, amount(candidates.get(0).analysis().seeds(), intermediate));
        assertEquals(1, amount(candidates.get(0).analysis().netOutputs(), target));
    }
    ...
    void discoversIdOnlyLoopThroughAnotherConcreteVariant() {
        ...
        assertEquals(1, candidates.size());
        assertEquals(2, candidates.get(0).members().size());
        assertEquals(1, amount(candidates.get(0).analysis().seeds(), returnedSeed));
        assertEquals(1, amount(candidates.get(0).analysis().netOutputs(), target));
    }
    ...
    private record FakeClosedLoopPattern(
        ...
        @Override public GenericStack[] getOutputs() {
            return closedLoopPayload.netOutputs().toArray(GenericStack[]::new);
        }
    }
