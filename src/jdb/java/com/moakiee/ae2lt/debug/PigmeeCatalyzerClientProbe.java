        ...
        try {
            ...
            switch (phase) {
                case 0 -> onServer(() -> {
                    ...
                    var host = (CrystalCatalyzerBlockEntity) level.getBlockEntity(POS);
                ...
                case 2 -> {
                    ...
                    require(!menu.getSlots(Ae2ltSlotSemantics.CRYSTAL_CATALYZER_MATRIX).get(0).isActive(),
                ...
                case 6 -> {
                    var menu = menu();
                    var output = menu.getSlots(SlotSemantics.MACHINE_OUTPUT).get(0).getItem();
                    if (output.isEmpty()) {
                    ...
                    require(menu.getSlots(Ae2ltSlotSemantics.CRYSTAL_CATALYZER_CATALYST).get(0).getItem().getCount() == 64,
                            "catalysts must remain in the client inventory");
                    capture("pigmee-catalyzer-complete-gui.png");
                    var recipe = EmiApi.getRecipeManager().getRecipe(new ResourceLocation(
                            "ae2lt", "crystal_catalyzer/quartz_block"));
                    require(recipe != null, "shared recipe missing from live EMI index");
                    require(EmiApi.getRecipeManager().getRecipe(new ResourceLocation(
                    ...
                    var shared = (CrystalCatalyzerRecipe) mc.level.getRecipeManager().byKey(
                            new ResourceLocation("ae2lt", "crystal_catalyzer/quartz_block"))
                            .orElseThrow();
