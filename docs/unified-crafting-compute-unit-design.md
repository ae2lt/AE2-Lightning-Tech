# 天枢 CPU 与物质扭曲矩阵统一计算单元设计

## 1. 文档目的

本文定义天枢合成 CPU 与物质扭曲矩阵共用的计算单元模型、主核心增幅规则、数值目标、结构限制和执行层预算语义。
...
| 单元 | 固定原始贡献 | 天枢 | 矩阵 |
|---|---|---:|---:|
| 发配单元 | `dispatchUnits + 1` | 可用 | 可用 |
| 增幅单元 | `amplifierUnits + 1` | 可用 | 可用 |
| 容量单元 | `storageUnits + 1` | 可用 | 禁止 |
| 冷却单元 | `coolingUnits + 1` | 禁止 | 可用 |
| 空白单元 | 无 | 可用 | 可用 |

外围单元类不得包含类似 `if (mainCore == OVERLOAD)` 的数值分支。扫描器只汇总原始数量或原始功率，所有等级换算集中在主核心配置和统一计算器中。
...
冷却单元始终只提供固定冷却点；距离衰减和热量公式属于矩阵主机，不属于冷却单元自身。
...
1. 同一个发配单元在扫描层始终只贡献一个 `dispatchUnits`，不读取主核心类型。
