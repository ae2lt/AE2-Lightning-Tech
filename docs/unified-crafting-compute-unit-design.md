...
因此第一次 `H` 完整接收后只能再次发送 `H`，同 tick 连续两次完整接收 `H` 后才允许尝试 `2H`；一旦完成该 tick 的双重基准证明，后续可直接按 `2H,4H,8H...` 倍增，不需要在每级之间额外重复当前批次。增长证明不能跨 tick 继承：上一 tick 的成功批次可作为本 tick 普通补货的历史基准 `H`，但本 tick 要上升时仍必须物理执行 `H,H,2H...`，不得执行 `H,2H...`。如果第二个 `H` 没有完整进入，本 tick 立即停止增长，第一份 `H` 不能与下一 tick 的一次 `H` 拼成证明；但下一 tick 可以重新开始一组全新的 `H,H,2H...`，不能因此永久退化为单 `H` 补货。`SIMULATE` 只做“该输入当前是否完全不可接收”的廉价预检，不用于推导机器槽位数、单槽容量或批量上限。
...
26. 每个目标、每个 canonical pattern 在每个 tick 开始增长时，都必须先在该 tick 物理执行 `H, H`，然后才能按 `2H, 4H, 8H...` 倍增。禁止 `H, 2H...`，但不要求 `H, H, 2H, 2H, 4H...`。最近 100 tick 内完整且无 overflow 的历史 `H` 只能作为本 tick 的起始基准，不能替代本 tick 双重基准证明中的任何一次 `H`。第二个 `H` 失败只终止本 tick，下一 tick 可以重新从新的 `H,H,2H...` 开始，不能把两 tick 的 `H` 拼成证明，也不能永久退化为单 `H`。
...
40. 无线 batch 在本 tick 第一份已证明批次 `H` 就真实拒绝时，下一 tick 先以 `H/2` 重试；若只是同 tick 双重基准证明中的第二份 `H` 拒绝，则按第 26 条仅终止本 tick，下一 tick 重新执行新的 `H,H,2H...`。连续两次成功提前探测确认机器更快时才额外降低失败压力，压力达到 7 后至少覆盖 3 tick。
