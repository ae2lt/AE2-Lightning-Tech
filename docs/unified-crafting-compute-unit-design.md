...
本节描述的调度启发式不能替代该验收；若实现符合下述伪代码却无法通过五组压力模型，仍必须调整实现。

过载样板供应器实现 `IBatchCraftingProvider`。批量探测不保存历史容量；每次选择一个目标后，都从一份开始，用本次调用内已经完整进入该目标的 copies 作为下一批上限：

```text
完整发配序列：1, 1, 2, 4, 8, ...
nextChunk = min(remaining, max(1, fullCopiesThisCall))
```

因此下一批永远不大于同一次调用中已经完整进入目标的数量。机器之前空闲时得到的结果不会被带到后续调用，避免历史大批量在机器忙碌、槽位变化或 capability 状态变化后把输入堵进余量队列。`SIMULATE` 只做“该输入当前是否完全不可接收”的廉价预检，不用于推导机器槽位数、单槽容量或批量上限。
...
无线均分不在每次 `pushBatch` 中扫描全部连接。它复用 ready queue，以旋转队首作为下一次起点，并对本次剩余 copies 做几何扇出：

```text
share = ceil(remaining / 2)
分配近似为 50%, 25%, 12.5%, ...
targetAttempts <= 1 + ceil(log2(maxCraft))
```

目标成功后移到队尾，下一次调用从后续目标继续；未使用份额直接留给后面的目标。这样短期不是绝对公平，但跨调用会轮转覆盖设备，而且单次目标选择复杂度只与 `maxCraft` 的对数有关，与无线连接总数无关。无线单目标模式保持粘性，只对当前队首目标执行一次完整爬坡。
...
不得用聚合后的物品总量重新执行一次向上取整计费，否则 batch 会改变原有每份样板的费用。一次 `pushBatch` 只消费一次共享 `D`，接受的每份 copy 各消费一份当前虚拟 CPU 的 `T`；内部爬坡和几何扇出不额外消费 `D`。
...
26. 每个目标、每个 canonical pattern 首次批量发配按 `1, 1, 2, 4...` 安全爬坡；后续复用最近 100 tick 内已证明的批次，避免每次从 1 重来。
...
32. 模糊样板只使用 CPU 当前解析出的具体输入；不存在跨变体历史容量复用。
...
35. 固定压力模型覆盖 512 个目标、单目标容量 512/2048、处理周期 1/5 tick 和单次处理 512 copies 的四种组合；随机模型固定容量 2048，独立生成 2..5 tick 处理间隔和 512..1024 copies 处理量。
36. 启动学习期取处理时间的十倍：1-tick 固定模型为 10 tick，5-tick 固定模型为 50 tick，2..5-tick 随机模型按最大值取 50 tick。启动期后，每个滑动 100-tick 窗口中固定模型持续达到至少 95% 理论吞吐，随机模型持续达到至少 80%；单 tick 和 20-tick 结果只作诊断。
37. 各模型启动期后，固定模型的每个 100-tick 滑动窗口总发配和单目标发配均不超过理想最少次数的 400%；随机模型对应上限为 800%。发配按实际 chunk push 次数统计，同一次目标选择内的 `1, 1, 2, 4` 计四次；`SIMULATE` 即失败、零接收和过大批次探测同样计一次，而同一 push 内部的 `SIMULATE -> MODULATE` 不拆成两次。
38. 对容量 2048、周期 5 tick、单次处理 512 的固定模型，理想发配是每目标每 20 tick 一次 2048；100 tick 内理想为五次，400% 上限为每目标二十次物理 push。
39. 五个模型均运行 200-tick 收敛测试和 5,000-tick 长稳态测试；不得以全程平均值掩盖启动期后任一 100-tick 滑动窗口的回退。
40. 无线 batch 的真实拒绝在下一 tick 以已证明批次的一半重试；连续两次成功提前探测确认机器更快时才额外降低失败压力，压力达到 7 后至少覆盖 3 tick。
...
- 过载样板供应器实现 `IBatchCraftingProvider`，加入无历史状态的倍增发配、跨调用旋转与单次几何扇出，以及普通 `sendList`/无线 overflow 的所有权结算。
