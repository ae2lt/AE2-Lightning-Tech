...
本文同时作为当前实现契约和后续调整依据。除明确标注为“待实现”的章节外，正文中的公式、结构限制、执行预算与状态字段已在当前代码中落地；后续修改实现时必须同步更新本文、玩家指南和验收测试。
...
自适应批量在十倍处理时间学习期后的持续吞吐与发配成本验收见
[《过载样板供应器自适应批量发配压力与收敛模型》](adaptive-batch-dispatch-stress-model.md)。
本节描述的调度启发式不能替代该验收；若实现符合下述伪代码却无法通过六组压力模型，仍必须调整实现。

过载样板供应器实现 `IBatchCraftingProvider`。每个“目标、canonical pattern”只保存最近一次完整进入且没有产生 `sendList`/overflow 的物理批次 `H`，把它作为后续 tick 的起始基准，而不把它解释成机器容量。首次基准为一份。每个 tick 选择该目标后，增长证明必须在本 tick 内重新完成：

```text
本 tick 增长序列：H, H, 2H, 4H, 8H, ...
禁止序列：H, 2H, 4H, ...
不需要的重复：H, H, 2H, 2H, 4H, ...
```

因此第一次 `H` 完整接收后只能再次发送 `H`，同 tick 连续两次完整接收 `H` 后才允许尝试 `2H`；一旦完成该 tick 的双重基准证明，后续可直接按 `2H,4H,8H...` 倍增，不需要在每级之间额外重复当前批次。增长证明不能跨 tick 继承：上一 tick 的成功批次可作为本 tick 普通补货的历史基准 `H`，但本 tick 要上升时仍必须物理执行 `H,H,2H...`，不得执行 `H,2H...`。如果第二个 `H` 没有完整进入，本 tick 立即停止增长，第一份 `H` 不能与下一 tick 的一次 `H` 拼成证明；单次失败不能直接让目标永久退化为单 `H` 补货。`SIMULATE` 只做“该输入当前是否完全不可接收”的廉价预检，不用于推导机器槽位数、单槽容量或批量上限。

单次第二个 `H` 失败仍只代表本 tick 不能增长；只有同一目标与 canonical pattern 连续观察到五次“第一份 `H` 完整进入、第二份 `H` 拒绝”，且五次的 `H` 和成功间隔完全相同，才可以把它解释为稳定蓄水池：机器按固定速度排空，供应器在实测间隔到期时只补一份已经证明的 `H`，不再为日常补货重复必败的第二份 `H`。该单份补货不是增长证明；以后若要扩大 `H`，仍须退出稳态并在同 tick 重新执行 `H,H,2H...`。

蓄水池稳态进入后先在实测到期点前一 tick 做有界校准。提前补货成功时立即把间隔缩短一 tick；提前补货失败时等待一 tick，在原到期点重试。连续两次提前失败即可确认固定周期并停止继续探测。若正常到期的 `H` 也失败，则间隔增加一 tick、解除固定周期确认并重新校准。每次完整进入的单 `H` 都必须刷新物理批次历史寿命，不能在持续工作 100 tick 后误过期并从 `1,1,2...` 重新冷启动。随机排空目标因此可以继续向更短间隔收敛，固定周期目标则只承担有限校准次数。

每个物理批次保持样板原始输入顺序。所有输入数量必须在第一次外部 `MODULATE` 前完成乘法溢出检查。某批次出现以下任一结果后停止对该目标继续爬坡：

- 零接收：该批次仍归 CPU，计入 leftover。
- 部分接收：整批所有权转移给目标或供应器余量队列，不再继续探测。
- 专用 `ICraftingMachine` 只接收一份：只按实际接收的一份结算，其余仍归 CPU。
- 能量不足：停止本次调用，已经发出的 copies 保持成功，未发出的 copies 返回 CPU。

零接收、部分接收以及产生 `sendList`/无线 overflow 的批次都不能提升历史 `H`。partial 已成功转移的部分正常计入 `ownedCopies`，但余量排空前不得向同一目标继续发配；排空后可以从最后一个完全无 overflow 的 `H` 恢复普通补货，但后续要增长时仍须在同一 tick 重新完整执行 `H,H,2H,4H...`。
...
```text
fullCopiesThisCall = 本次对当前目标完整进入的 copies，仅用于决定下一批大小
ownedCopies        = 目标或供应器已经取得所有权、CPU 不得重新注入的 copies
leftoverCopies     = maxCraft - ownedCopies
```

普通相邻模式把部分批次的精确余量放入原版 `sendList`；无线模式放入对应连接自己的 overflow bucket。只要某批次已有任意输入进入目标，其余输入就由供应器接管，该批次全部计入 `ownedCopies`，避免 CPU 重注入造成复制。

无线均分复用 ready queue 和最近 100 tick 的成功 ownership 统计。一次 pass 只向当前活跃目标各发放至多一次租约，并优先选择窗口内取得 copies 最少的目标；冷却中或持有 provider-owned overflow 的目标不参与当前活跃平均值。每个目标先取得公平调度器计算的基础 allowance；若完整执行当前目标的安全 ramp 需要更大 allowance，则只允许在不超过“剩余 copies / 本 pass 尚未访问的活跃目标数”向下取整值时抬高租约：

```text
targetAllowance = min(remaining, fairnessAllowance)
equalShareFloor = floor(remaining / remainingTargetsInPass)
if rampAllowance > targetAllowance && rampAllowance <= equalShareFloor:
    targetAllowance = rampAllowance
同一 pass 中每个目标最多取得一次租约
```

租约内允许目标按本节的 `H,H,2H,4H...` 序列消耗 allowance，未使用份额留给后续目标。目标成功后更新其 100-tick ownership 统计，下一份租约继续选择当前最少者；恢复可用的目标从当前活跃平均值起步，不追赶冷却期间的历史欠账。这样有限请求会被均匀拆给目标，持续大请求则按实际处理能力自然获得更多补货。无线单目标模式保持粘性，只对当前目标执行一次完整爬坡。

非 batch 的单份无线发配同样不得按连接总数扫描；每次最多探测两个队首候选，失败或受罚目标轮转/退避后由后续调用继续推进。

冷却接受两类明确背压证据：一份真实发配零接收，或发配后确实留下 `sendList`/无线 overflow。更大批次零接收但没有转移所有权时只停止当前目标并轮转，不因“尝试额度较大”额外惩罚。普通模式由原版 `sendList` busy 状态阻止新发配；无线模式在 bucket 存在期间移出 ready queue，并同时进入现有失败冷却，避免 bucket 很快清空后立刻再次塞满。死亡连接、未加载目标和全局能量不足继续沿用现有硬失败/全局中止路径。

过载样板供应器的附加传输费用保持逐份语义：

```text
providerPowerCost = oneCopyPowerCost × ownedCopies
```

不得用聚合后的物品总量重新执行一次向上取整计费，否则 batch 会改变原有每份样板的费用。一次 `pushBatch` 只消费一次共享 `D`，接受的每份 copy 各消费一份当前虚拟 CPU 的 `T`；内部爬坡和同一 pass 的目标租约不额外消费 `D`。
...
模糊样板直接使用 CPU 已经解析出的具体 `AEKey` 发配；由于不保存历史容量，不存在不同组件变体共享容量记录的问题。过载样板只有在动态输出登记、返回过滤和解锁状态能够按 `ownedCopies` 正确放大时才能进入该路径；否则仍逐份执行。
...
26. 每个目标、每个 canonical pattern 在每个 tick 开始增长时，都必须先在该 tick 物理执行 `H, H`，然后才能按 `2H, 4H, 8H...` 倍增。禁止 `H, 2H...`，但不要求 `H, H, 2H, 2H, 4H...`。最近 100 tick 内完整且无 overflow 的历史 `H` 只能作为本 tick 的起始基准，不能替代本 tick 双重基准证明中的任何一次 `H`。单次第二个 `H` 失败只终止本 tick，不能把两 tick 的 `H` 拼成证明；只有连续五次相同 `H`、相同间隔的第一份成功/第二份失败样本才允许进入单 `H` 蓄水池补货，且该模式不能被当作增长证明。
27. 第一批完整进入、后一批部分进入并产生 `sendList`/overflow 时，CPU 按所有权转移份数结算，不重新注入已经转移的材料。
28. 更大批次零接收或部分接收后立即停止当前目标；已证明批次零接收时下一次先尝试其二分之一，成功后恢复已证明批次，留下 `sendList`/overflow 时进入失败冷却。
29. 无线均分以每目标、每 canonical pattern 最近 100 tick 成功取得所有权的 copies 为调度指标；拒绝冷却或 provider-owned overflow 中的目标不参与活跃平均值。
30. 相邻和无线目标都保持样板输入原始顺序；只有最后一个物理批次可能处于部分发配状态。
31. 无线均分优先选择 100-tick 窗口内取得 copies 最少的活跃目标；同一 pass 中每目标最多租约一次，恢复可用的目标从当前活跃平均值开始，避免积累不可用期间的追赶债务。
32. 模糊样板始终使用 CPU 当前解析出的具体输入，不能缓存或复用旧变体的物品 key。由于同一输入槽各变体的每份数量一致，历史 `H` 只记录 copies 数，可以由同一 canonical pattern 共用；本 tick 的 `H,H` 完整接收证明负责重新验证当前具体变体，失败即按正常回退处理。
33. `pushBatch` 接受多份 copies 时只消费一次 `D`，按 `ownedCopies` 消费 `T` 和供应器附加能源；全部拒绝时不消费 `D/T`。
34. 第二种输入返回部分接收时，已转移材料不会同时由 CPU 重新注入；该物理批次停止当前目标的后续倍增。
35. 固定压力模型覆盖 512 个目标：A-D 为单目标容量 512/2048、处理周期 1/5 tick、单次处理 512 copies 的四种组合；E 为容量 576、每 20 tick 处理 9 copies；随机模型 R 固定容量 2048，独立生成 2..5 tick 处理间隔和 512..1024 copies 处理量。
36. 启动学习期取处理时间的十倍：1-tick 固定模型为 10 tick，5-tick 固定模型为 50 tick，模型 E 为 200 tick，2..5-tick 随机模型按最大值取 50 tick。启动期后，每个滑动 100-tick 窗口中固定模型持续达到至少 95% 理论吞吐，随机模型持续达到至少 80%；单 tick 和 20-tick 结果只作诊断。
37. 各模型启动期后，固定模型的每个 100-tick 滑动窗口总发配和单目标发配均不超过理想最少次数的 400%；随机模型对应上限为 800%。模型 E 的理想最少次数应用每目标每 100 tick 一次物理 push 的存活保底，因此上限为每目标四次、全体 2,048 次。发配按实际 chunk push 次数统计，同一次目标选择内的 `1, 1, 2, 4` 计四次；`SIMULATE` 即失败、零接收和过大批次探测同样计一次，而同一 push 内部的 `SIMULATE -> MODULATE` 不拆成两次。
38. 对容量 2048、周期 5 tick、单次处理 512 的固定模型，理想发配是每目标每 20 tick 一次 2048；100 tick 内理想为五次，400% 上限为每目标二十次物理 push。
39. A-D 与 R 运行 200-tick 收敛测试，模型 E 因启动学习期为 200 tick，收敛测试至少运行 300 tick；六个模型均运行 5,000-tick 长稳态测试。不得以全程平均值掩盖启动期后任一 100-tick 滑动窗口的回退。
40. 无线 batch 在本 tick 第一份已证明批次 `H` 就真实拒绝时，下一 tick 先以 `H/2` 重试；若只是同 tick 双重基准证明中的第二份 `H` 拒绝，则按第 26 条记录蓄水池候选样本。普通节拍的连续两次成功提前探测仍用于确认机器更快并降低失败压力；已经进入蓄水池模式时，一次完整成功的提前单 `H` 即可把补货间隔缩短一 tick，连续两次提前失败则停止固定周期的后续探测，正常到期失败会重新打开校准。
41. 压测报告必须同时输出全局和单目标最坏 100-tick 发配指标；20-tick 指标只作诊断，不参与通过判定。
42. 六组压力模型进入稳态后清空调度公平计数、节拍学习和目标物理批次历史，同时保留机器库存；再次给予对应模型十倍处理时间后，紧接着的完整 100-tick 窗口必须重新满足第 36、37 条的吞吐和发配门槛。
...
- 过载样板供应器实现 `IBatchCraftingProvider`，加入按目标与 canonical pattern 保存安全基准、每 tick 内 `H,H,2H,4H...` 增长证明、100-tick ownership 公平调度，以及普通 `sendList`/无线 overflow 的所有权结算。
