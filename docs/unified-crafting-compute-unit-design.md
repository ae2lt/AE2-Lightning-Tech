...
26. 每次 `pushBatch` 对每个目标都从 `1, 1, 2, 4...` 重新爬坡，不读取或写入历史容量。
27. 第一批完整进入、后一批部分进入并产生 `sendList`/overflow 时，CPU 按所有权转移份数结算，不重新注入已经转移的材料。
28. 更大批次零接收或部分接收后立即停止当前目标；零接收只轮转，留下 `sendList`/overflow 时进入失败冷却。
29. 无线均分按 `50%, 25%, 12.5%...` 几何扇出，单次访问目标数不超过 `1 + ceil(log2(maxCraft))`，不随设备总数线性增长。
30. 相邻和无线目标都保持样板输入原始顺序；只有最后一个物理批次可能处于部分发配状态。
31. 下一次无线均分从旋转后的队首继续，长期近似公平但不维护绝对份额或 `vruntime`。
