...
两种机器共用原有加工配方。配方的能耗、闪电与基础输入输出保持原值；猪咪版在机器执行层忽略能耗并应用上述固定规则。没有 `pigmee_*` 加工配方或配方变体字段。旧存档中的 `pigmee_*` 加工 ID 会迁移到对应原配方，保留进度和产物快照。
...
- 加工中 NBT 保存恢复，旧正能耗/零能耗的 `pigmee_*` 快照 ID 迁移，拆除掉落保留催化剂与产物。
...
客户端探针通过真实菜单网络包倒入一桶水，验证空桶返回、模式锁定、无 FE 进度同步、产物/水量/催化剂数量，再打开实时 EMI 原配方，确认仍声明 100000 FE、1 个高压闪电、1 个催化剂及 1 个基础产物，且没有零能耗猪咪副本。结果写到游戏目录下 `pigmee-catalyzer-client-probe.txt`，原始游戏截图位于其 `screenshots` 目录。
...
后续按“只是机器不耗能，不能修改加工配方”的澄清完成修正：17 个原加工配方 JSON 与引入猪咪前的主线逐字节一致；配方类型、查询和 JEI/EMI 展示恢复原行为，移除 9 条猪咪副本。重新通过 6 项 GameTest、完整依赖下的 910 项单元测试及客户端探针。实际 EMI 显示 100k FE、1 个高压闪电、1 秒；同一原配方在猪咪机器中实际消耗 0 FE / 0 闪电，并正确同步 15 秒加工进度和固定产出。
