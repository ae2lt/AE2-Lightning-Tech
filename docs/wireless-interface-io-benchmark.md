...
| 256 目标冷热/脉冲/突发鲁棒性集成验收 | 是 | `runGameTestServer`；稳态吞吐、恢复延迟与原始受阻率分开记录，见 16.9 |
...
2026-09-05 最新工作树基于 `5e7ab8e1`，结果与用户授权的冷热验收口径见 **16.9**。
818 个普通测试、完整模型与 20,000 tick 长稳模型，以及普通 GameTest 入口已通过。
FAST 到期访问不再被旧空缓存遮蔽，NORMAL 速度模式仍保留 20 tick 空缓存。
下面保留的是 2026-09-04 的历史阶段记录，不代表最新验收结果。
...
### 16.9 2026-09-05 实际观察期限、热路径开销与冷热验收

本轮由用户授权继续优化，目标为保持吞吐与语义正确，降低搬运延迟和 MSPT、减少
掉 TPS。用户同时要求评估生产受阻率是否过于严格。本轮未使用子代理，生产基线
固定为 `5e7ab8e1c1808c99bc65497b1a7102460d46bb3c` 的独立检出。

#### 修复与验证

- FAST 时间轮已经安排了 1～5 tick 的服务，但旧 `runExtract`/probe 可用 20 tick
  前的空缓存替代实际观察。本轮让 FAST 到期访问实际观察容器；NORMAL 速度模式的
  20 tick 空缓存保留，非空观察仍不能代替下一次扫描。未改变时间轮、相位、catch-up
  或每次允许搬运的数量。
- 删除每次扫描构建、复制并保留最多 256 个键的列表。生产代码只用其空/非空状态，
  因此现在只记录空状态和扫描时间。没有采用历史上失败的“等到缓存 TTL 再唤醒”。
- 缓冲变化每接口每 tick 只调用一次 `saveChanges`。AE2 在 tick 末尾执行已排队的
  变更标记，但原先每次调用仍查找区块；加载/清空缓冲会重置合并状态。
- `addToImportBuffer` 的调用都来自正在执行的 grid I/O。AE2 明确忽略对当前 tick
  节点的 `alertDevice`，ProxyTicker 已返回 URGENT，因此移除逐键的无效唤醒调用。
  能源核算、真实抽取/插入、溢出所有权和拒收分片算法未改动。

新增 `fastImportColdOutputAllPhases`：20 个真实桶分别在连续 20 个 tick 产生带独立
键的批次，覆盖旧空缓存的全部相位；直接读取远端和 ME 网络，分别记录取走与入网
时间，逐键核对数量。这些观测仅在普通 GameTest 运行，不进入正式性能计时。

同一份新夹具分别运行旧生产代码和候选，结果如下（tick）：

| 指标 | 5e7ab8e1 | 候选 |
|---|---:|---:|
| 冷启动产出→取走最大等待 | 20 | 5 |
| 冷启动产出→ME 网络最大等待 | 21 | 6 |
| 256 目标冷热产出→取走 P99 | 19 | 5 |
| 256 目标冷热产出→取走最大等待 | 24 | 5 |
| 最差目标稳态吞吐 | 94.4444% | 100% |
| 原始生产受阻率 | 13.4084% | 5.0115% |
| 最长连续受阻机会 | 8 | 4 |
| 总实际产出 | 49,413,888 | 54,205,632 |

旧代码在新的冷启动与冷热验收中仍失败；候选的 815 个普通测试、25+178 完整模型、
25+181 长稳模型（含三个 20,000 tick 场景）及全部 7 个 GameTest 通过。此前仅缓存
修改的候选也已通过完整模型和 GameTest，其原始受阻率为 6.1894%；真实测试位置/
相位会影响该比率，不能将两次候选之间的差异归因于存档合并。

#### 为什么调整冷热用例的受阻率判断

这个用例每目标只有一批输出容量，共 129 次计划生产机会。每个目标只错过一次，
全局受阻率也达到 `1/129 ≈ 0.775%`，已经高于原来的 0.1%，而时间轮本来允许有界
的恢复等待。因此仅针对这个冷热用例，将旧 0.1% 作为 `legacyBlockedGate` 诊断
完整输出，使用下列明确的行为条件验收：

- 生产时间线、目标数、槽位和批次数量不变，原始机会、产出、受阻均保留；
- 两个连续生产区段在已知切换后的六 tick 观察窗口外，即 `[46,80)`、`[306,380)`，
  每个目标的稳态吞吐至少 99%；最长连续受阻不超过五次生产机会；
- 全部实际产出的最大取走等待不超过六 tick，原有孤立 pulse 的五 tick 条件保留；
- 原有所有权、最终排空与键数检查保留；新冷启动测试还约束实际入网等待不超过
  十一 tick（五 tick watchdog + 五 tick flush + 一 tick 观测余量）。

这不是通用放宽其它基准：连续负载、高基数、确定性矩阵和回归保护的门槛均未改动。
旧生产代码即使应用新口径，仍有 24 tick 的等待、8 次连续受阻以及不足 99% 的稳态
吞吐；候选对这些指标有实际改善。

原始产物目录：`benchmark-results/scheduling-20260905/`。`baseline-latency.log` 保留
旧生产代码的反例，候选日志为 `build/wireless-scheduling-candidate2-validation.log`。
生产源文件和测试夹具的 SHA-256 位于 `candidate-source-hashes.json`，跟踪文件补丁
保存在 `candidate.patch`。正式性能结果单独记录，不能由上述 tick 延迟推导毫秒收益。

#### 首轮计时与新的分配证据

上述缓存/存档清理候选完成了五对正式 1024×27 连续负载运行。压力组中位数 mean
MSPT 为 `44.854484 → 43.651053`，P99 为 `55.5336 → 55.7825`，I/O P99 为
`41.6762 → 41.0004`，>50 ms 比率为 `9.6667% → 9.0000%`。比较器返回
`FAIL / NO_MEASURABLE_IMPROVEMENT`，不能据此宣称显著降低了 MSPT。
报告保存在 `comparison-hot/`，原始目录 `baseline-hot/`、`candidate-hot/` 未覆盖。

随后单独运行 35 秒稳定生产段 JFR，文件为 `profile-current.jfr`，不混入正式比较。
用 `JfrSummary.java` 通过 JFR consumer API 读取完整栈；`jfr print` 默认仅输出五层
栈，不能用那种截断输出判断接口调用归属。完整汇总为 `profile-full-stack-summary.txt`。
1,820 个服务端 CPU 样本中，1,362 个位于接口调用链；接口 CPU 样本的 31.1% 在
`ServerChunkCache.getChunk`。完整栈也表明真实容器抽取会引发邻居更新，不能把这些
成本全部归给夹具，或据此省略具有游戏语义的容器回调。

分配采样权重中，接口路径约 28.8% 是 `scanImportKeys → KeyCounter.add` 创建的
`Object2LongOpenHashMap` 数组/对象。基于这个新证据，候选增加 `ImportScanBuffer`：

- 每次取得缓冲都清空数量，仍实际执行 `getAvailableStacks`，没有跨 tick 复用库存；
- 完成扫描后清理不再出现的 primary type，避免不同目标令空子表永久增长；
- 超过 256 个键的扫描完成后放弃复用，避免大 map 拖慢后续小扫描；这个数只限制
  保留的缓存，不限制本次扫描或抽取数量；
- 嵌套调用使用独立临时计数器，`finally` 归还外层计数器，避免覆盖进行中的扫描。

这与历史上的无边界 `clear()` 候选不同。新增三个测试覆盖不同目标/数量、257 个
变体边界和嵌套调用。该候选通过 818 个普通测试、长稳模型与普通 GameTest；冷启动
最大取走/入网等待仍为 5/6 tick，冷热最差目标稳态吞吐为 100%。本次冷热原始受阻率
为 3.0039%，P99/最大取走等待为 4/5 tick；相位位置变化使其不能与前一候选的受阻率
直接作单一优化归因。

该生产源文件 SHA-256 为 `7C70414CCB8E310CC85E03A4013B5D9CCA800A96B6A430770D866335539E5FC2`，
对应 `candidate3-source-hashes.json` 和 `candidate3.patch`；性能运行单独写入
`candidate3-hot/`，不复用首轮候选的报告。

#### 最终五轮结果与保留边界（2026-09-06）

同一台 i7-13650HX、20 逻辑处理器、Windows 11、Balanced 电源策略、Java 21.0.11、
Gradle 8.8；基线与最终候选各 5 个 control/stress 对，每次独立 JVM，200 tick 预热、
1200 tick 采样。生产代码在整组运行期间固定。所有正式报告均为 schema 3、
`sample-ticks-only` GC 窗口。新增诊断用例在正式入口跳过，计时用的 1024×27 连续
生产夹具与原始基线相同。
...
| 指标 | 5e7ab8e1 基线 | 最终候选 |
|---|---:|---:|
| mean MSPT | 44.854484 ms | 43.858562 ms |
| P95 MSPT | 51.6502 ms | 50.6293 ms |
| P99 MSPT | 55.5336 ms | 55.2867 ms |
| 无线 I/O P99 | 41.6762 ms | 41.1659 ms |
| 超过 50 ms 的 tick | 116/1200（9.6667%） | 77/1200（6.4167%） |
| GC 时间 | 722 ms | 521 ms |
| 峰值已用堆 | 775.826 MiB | 689.672 MiB |
| 每轮最大 MSPT 的中位数 | 122.4506 ms | 142.1873 ms |

长 tick 数中位数下降约 33.6%，GC 时间下降约 27.8%，堆峰值下降约 11.1%。平均
MSPT 只下降约 2.2%，P99 基本持平。最坏一轮 P99 为 `55.8630 → 61.3714 ms`，
五轮最大单 tick 为 `135.4071 → 165.5434 ms`：**极端峰值没有改善**，不能被长 tick
总次数或 GC 总时间下降掩盖。

正式比较结果仍是 `FAIL / NO_MEASURABLE_IMPROVEMENT`。其可测改进分类依据 mean/
P99/I/O P99，当前降幅未达到容差外改善；绝对 MSPT 预算也未通过。比较器另外明确
记录了 >50 ms 比率、GC 比率和堆峰值的容差外改善，没有改变比较器来取得 PASS。
按本轮用户的综合目标保留这个候选：实际等待与稳态吞吐改善、长 tick 频率和 GC
下降，但不宣称完成严格 MSPT 验收，也不把 `capacityTps=20` 当作实测 TPS 提升。

报告为 `comparison-candidate3-hot/live-comparison.md`，完整中位数/最差值汇总为
`final-metric-summary.json`。末尾另外跑了一对旧生产代码，结果 mean/P99 为
`44.484536/55.1357 ms`、I/O P99 为 `42.4369 ms`、>50 ms 为 `92/1200`、GC 为
`688 ms`，处于原基线附近。它仅用于环境回查，单独保存在 `baseline-recheck-hot/`，
没有替换原五轮数据或混进中位数。

最终真实高基数拒收恢复诊断也通过：最大缓冲 `27,648` 键，产出、抽取、入网均为
`12,386,304`，结束时接口缓冲和远端剩余都为 `0`；网络 tick 320 恢复，缓冲首次排空
在 tick 326。原始源端受阻事件 `238,592` 保留，说明背压仍有效；此场景不是等负载
性能对照。JSON/CSV 为 `final-high-cardinality.json` 与 `final-high-cardinality-ticks.csv`。
手工诊断启动没有覆盖探针默认的 Git/dirty 属性，实际工作树身份与生产哈希记录在
`final-validation-provenance.json`；这份诊断未用于正式性能比较。

最终普通测试 818 个、0 failures/errors，完整/长稳模型通过，普通 GameTest 入口和
高基数拒收恢复入口通过。生产文件与已计时快照哈希一致，源码快照另存于
`candidate3-source/`。本轮没有测试所有第三方容器或证明最坏 tick 的改善；下一步若
继续降低 MSPT，应针对已采样的容器抽取/邻居更新路径研究可保持其可观察行为的改进，
而不是继续放宽门槛或省略原有回调。
